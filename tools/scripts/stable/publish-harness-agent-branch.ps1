...
function Invoke-GovernanceCheck {
    ...
    $scriptPath = Join-Path $RootPath "tools/scripts/stable/test-harness-governance.ps1"
    if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
        throw "Governance self-check script missing: tools/scripts/stable/test-harness-governance.ps1"
    }
