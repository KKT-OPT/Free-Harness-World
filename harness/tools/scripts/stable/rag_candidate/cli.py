...
def build_parser() -> argparse.ArgumentParser:
    ...
    ingest.add_argument("--raw-domain", default=None, help="domain/source folder for copied raw files; default uses run id")
    ingest.add_argument("--copy-raw", action="store_true", help="copy source files into user/knowledge/raw/<domain-or-run-id>")
    ingest.add_argument("--max-chunk-chars", type=int, default=4000)
    ingest.add_argument("--extraction-granularity", default="standard", choices=["fine", "standard", "coarse", "minimal", "custom"])
    ingest.add_argument("--entity-cap", type=int, default=None)
    ingest.add_argument("--concept-cap", type=int, default=None)
    ingest.add_argument("--batch-strategy", default="single-pass-local-conversion")
    ingest.add_argument("--tag-vocabulary-mode", default="default", choices=["default", "custom"])
    ingest.add_argument("--allowed-tags", nargs="*", default=[])
