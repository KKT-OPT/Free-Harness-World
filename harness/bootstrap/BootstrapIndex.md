...
version: v1.0.0-h8-bootstrap-foundation
...
review:
  ...
  decision: h8-bootstrap-foundation
---
# Bootstrap 索引
...
`-Mode init` 只创建缺失的本地目录和本地 registry 文件，不覆盖已有文件，不写入凭据，不导入真实项目。
...
| Init 命令 | `bootstrap-harness-workspace.ps1 -Mode status` 可运行并输出 JSON 摘要。 |
